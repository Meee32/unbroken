...
// Copyright (c) 2013 UnbrokenCoin developers
...
const string strMessageMagic = "UnbrokenCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1)
        nSubsidy = 2000000 * COIN; // 2 million premine so that Jimmy can make bounties to support the coin without going broke!
    else if (nHeight >= 2 )
...
static const int64 nTargetTimespan = 4 * 60; // Retargets every 2 blocks not 2016 so that it doesn't take a YEAR to retarget!!!
static const int64 nTargetSpacing = 2 * 60; // 2 minutes blocks so that Unbrokencoin is ACTUALLY FASTER than Bitcoin!!!
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnbrokenCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnbrokenCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("UnbrokenCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running UnbrokenCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnbrokenCoinMiner terminated\n");
        throw;
    }
}
